...
def get_papers_by_keyword(keyword):
    ...
    europmc_query = config.get("DEFAULT", "EUROPE_PMC_KEYWORDS_QUERY").format(
        keyword=keyword
    )
    ...
    europmc_results = [
        dict(
            chain(
                {"keyword": keyword, "source": "europmc"}.items(),
                parse_result(r).items(),
            )
        )
        for r in europmc_results
    ]
...
def get_citing_papers(pmid):
    ...
    europmc_results = [
        dict(
            chain(
                {"cites": [pmid], "source": "europmc"}.items(), parse_result(r).items()
            )
        )
        for r in europmc_results
    ]
...
def get_paper_by_pmid(pmid):
    ...
    if "resultList" in europmc_results:
        results = europmc_results["resultList"]["result"]
...
def execute_query(query, cursor_mark="%2A"):
    ...
    europmc_opts = config.get("DEFAULT", "EUROPE_PMC_QUERY_OPTS").format(
        cursor=cursor_mark
    )
